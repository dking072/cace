#!/usr/bin/env python
# coding: utf-8
"""Training script for CACE + LES long-range interactions on the SPICE
dipeptides dataset.

Model architecture matches ``caceles-quads.py`` (TensorReadout -> LesWrapper
with quadrupoles + SR Atomwise), using the non-lightning 4-stage progressive
training schedule. Data is the same SPICE dipeptides train/val/test split used
by ``cace-lr-fit/fit-dipeptides/fit-dipeptides.py``.
"""
...
# Per-element reference energies (eV), fit by least squares on the 495 training
# frames via cace.tools.compute_average_E0s. Subtracting these shrinks the
# target energy scale so Stage 1 doesn't have to learn the atomization offset.
atomic_energies = {
...
# dipep XYZ uses `force` (singular) in the per-atom arrays and `energy` in info.
